...
export const useDynamicBgStyle = (imageUrl: string) => {
  ...
  useEffect(() => {
    const updateBackgroundStyle = async () => {
      ...
      const isBright = brightness > 200; // Adjust threshold as needed
      setBgStyle({
        filter: "blur(500px)",
        opacity: isBright ? 0.2 : 0.5, // Lighter glow for bright images
      });
    };

    if (imageUrl) updateBackgroundStyle();
  }, [imageUrl]);
